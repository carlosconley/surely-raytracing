...
pub fn write_color<W: Write>(
    ...
    let rgb = rgb
        .map(|x| x * scale)
        .map(|linear| match exposure_val {
            Some(val) => exposure(linear, val),
            None => linear,
        })
        .map(linear_to_gamma)
        .map(|rgb| (256. * intensity.clamp(rgb)) as u8);

    writeln!(out, "{} {} {}", rgb[0], rgb[1], rgb[2]).expect("Error writing pixel to buffer");
}
